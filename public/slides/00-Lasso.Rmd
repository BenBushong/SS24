...
subtitle: "SSC442; Spring 2024"
author: "Courtesy of Prof. Ed Rubin (Oregon)"
...
2. Estimate the linear models via .pink[least squares]
...
__How could shrinking coefficients twoard zero help or predictions?__

--

_Remember we're generally facing a tradeoff between bias and variance._
...
.grey-light[(Perhaps you saw that coming. This will be a consistent theme.)]

---
class: MSU
# Penalization

Because we sum the .b[squared] coefficients, we penalize increasing .it[big] coefficients much more than increasing .it[small] coefficients.

For a value of $\beta$, we pay a penalty of $2 \lambda \beta$ for a small increase..super[.pink[†]]
...
__Predictors' .hi[units] can drastically .hi[affect ridge regression results].__
...
Supples we take an example where $x_1$ denotes distance.
...
Let's use to the credit dataset from the ISLR package.

We have 11 predictors and a numeric outcome `balance`.
...
_Can't we just drive the coefficients to zero?_
...
Everything else will be the same. Sort of... 
...
__Lasso coefficents__ for $\lambda$ between 0.01 and 100,000
...
__Cross-validated RMSE and__ $\lambda$: Which $\color{#8AA19E}{\lambda}$ minimizes CV RMSE?
