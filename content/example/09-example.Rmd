...
## Our data and goal
We want to use the `wooldridge::wage2` data on wages to generate and tune a non-parametric model of wages using a regression tree. Use `?wage2` (after you've loaded the `wooldridge` package) to see the data dictionary. 
...
I'm going to assign you to groups of 3-4 to work on live-coding. One person in the group will need to have Rstudio up, be able to share screen, and have the correct packages loaded (`caret`, `rpart`, and `rpart.plot`, plus `skimr`). Copy the `rmse` and `get_rmse` code into a blank R script (you don't have to use RMarkdown, just use a blank R script and run from there).
...
## F22: We already went thru this yesterday
