...
linktitle: "10: Nonparametric Regression"
...
## Setup 
We are going to use the `wooldridge::wage2` data on wages to generate and tune a non-parametric model of wages using a regression tree (or decision tree, same thing). Use `?wage2` (after you've loaded the `wooldridge` package) to see the data dictionary. 
...
For the first fifteen minutes, all I want you to do is the following. We are trying to predict `wage` for this exercise:

- Estimate a decision tree on `wage_clean` using the default parameters (use the whole dataset for now, we'll train-test on the problem set). 
...
Again, we'll spend about 15 minutes on this. Remember, you can use `?wage2` to see the variable names. Make sure you know what variables are showing up in the plot and explaining `wage` in your model. You may find something odd at first and may need to drop more variables...
...
### Connection to Previous Lab(s)

In a previous lab (on linear models) you were asked to make increasingly complex models by adding variables. In regression trees and kNN, complexity comes from a tuning parameter. But many people have asked about ways of doing Lab 7, so here are two possible solutions.
...
## Part 2: Looping through the Trees
...
## Part 3 (if time): Evaluating your Trees
