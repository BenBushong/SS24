...
Welcome to our last class. Please make sure you pay close attention to the requirements for the final project. 

# More Geospatial in `R`
We went over the basics and (hopefully) got some practice using geospatial data in R. Let's now look closely at the `tidycensus` packages. These offer a way to get census data and attach it to geospatial data---a common practice in social science research.

To help you understand the relationship between packages, let's briefly zoom out. Thus far we have used `tigris` to get TIGER/line census boundaries for things like counties. Although this can be helpful, `tidycensus` does more than this -- we can pull actual US Census Data with its geospatial references (the polygons referring to the counties, tracts, block groups, and blocks). It's helpful to do this in one step and getting geospatial references *along with* census data is possible because `tidycensus` interfaces with `tigris` to combine the census data and the census geographies.
...
As you can see, we get some counties that extend into the Great Lakes. That's okay. We could use `st_intersect` with a map of the Great Lakes to clean that up as illustrated last class, but I'll leave that as an exercise for you. 
...
Finally, let's show all the Ingham County tracts. I've changed the color mapping for the `col` aesthetic so that the tract that holds MSU will be colored in the appropriate green:


```{r, echo = TRUE}
ggplot(Ingham.tracts) +
  geom_sf(aes(fill = TRACTCE10), color = 'gray20') +  # Apply grey scale based on TRACTCE10
  scale_fill_grey(start = 0.8, end = 0.5) +           # Grey scale fill for all tracts
  geom_sf(data = Ingham.tracts[Ingham.tracts$NAME10 == '9800', ], fill = '#18453b', color = '#18453b') +
  theme_minimal() +
...
Try it! This is your 14th and final Weekly Writing *or* Lab, due on **MONDAY** If you need an additional Weekly Writing, you must do this. If you need an additional lab, you must do this.
