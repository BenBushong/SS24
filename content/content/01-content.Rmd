...
linktitle: "1: The Tidyverse"
read_date: "2022-09-06"
lastmod: "`r Sys.Date()`"
menu:
  content:
...
- Weekly Writing on Saturday night, Lab on Monday night.  
  - Repeat every week
...
```{r show-slide-tabs, echo=FALSE, results="asis", warning=F, message = F}
...
### Teams
Please form teams of 3-5 people. Once all agree to be on a team, have one person email our TA, Sam, tucke130@msu.edu **and cc all of the members of the team** so that nobody is surprised to be included on a team. Title the email *[SSC442] - Group Formation*. Tell us your team name (be creative), and list in the email the names of all of the team members and their email address (in addition to cc-ing those team members on the email).

If you opt to not form a team, you will be automatically added to the "willing to be randomly assigned" pool and will be paired with another 3-4 persons.

Send this email by **Sunday September 18th** and we will assign un-teamed folks at the beginning of the following week. Project 1 is due October 15th. See [schedule](/schedule/) for all the important project dates.
...
- Teams will submit only **one** polished write-up for the mini-projects and the final

To combat additional freeloading, we will use a reporting system for work contributed. More details will follow. 
...
Imagine a bag of marbles with two types of marbles: `r emo::ji('club suit')` and `r emo::ji('diamond suit')`.

We are going to pick a **sample** of $n$ marbles (with replacement). 

We want to learn something about $\mu$, the **objective** probability to pick a `r emo::ji('club suit')`.

In addition to defining the **objective** probability of picking a `r emo::ji('club suit')`, we have an observed fraction $\eta$, which will define as the fraction of `r emo::ji('club suit')` in the *sample*.

**Question**: Can we say anything exact and for-sure about $\mu$ (outside the data) after observing $\eta$ (the data)?

- No. It is possible for the sample to be all `r rep(emo::ji('club suit'), 5)` even when the bag is is 50/50 `r emo::ji('club suit'); emo::ji('diamond suit')`

  - No matter what we draw, we can't (based on that draw alone) eliminate the possibility of drawing a `r emo::ji('diamond suit')`.
  
  - And unless we assume that the only two values in the world are `r emo::ji('diamond suit')` and `r emo::ji('club suit')`, we can't rule out `r emo::ji('poop')`^[I discovered the `emo::ji()` function at 8:55am. My wife joked that I would find a way to use the poop emoji by 9:00am. It is now 8:59am. She was right.]
...
**Hoeffding's Inequality** states, loosely, that $\eta$ (the fraction of our sample that is `r emo::ji('club suit')`) cannot be too far from $\mu$.

$$\mathbb{P}\left[|\eta - \mu| > \epsilon \right] \leq 2e^{-2\epsilon^2n}$$
$\eta \approx \mu$ is called **probably approximately correct** (PAC) learning.
