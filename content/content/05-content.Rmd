...
- The correlation between two variables tells us how they move together. How is this helpful? 
- Sample correlation is a random variable. What does this mean?
- What is the main intution for why we might want to run a linear model? 
- How do we justify linear models? 
...
By way of illustration, let's assume that the `r nrow(galton_heights)` pairs of fathers and sons is our `r kableExtra::text_spec("entire population", color = 'red')`. A less fortunate geneticist can only afford measurements from a random sample of 25 pairs. The sample correlation can be computed with:
...
1. Load the `GaltonFamilies` data from the __HistData__. The children in each family are listed by gender and then by height. Create a dataset called `galton_heights` by picking a male and female at random from each family (hint: `group_by` appropriately and use `sample_n`).

2. Make a scatterplot for heights between mothers and daughters, mothers and sons, fathers and daughters, and fathers and sons. You may have to use `gridExtra` or `patchwork`)
