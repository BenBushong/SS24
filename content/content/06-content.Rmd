...
linktitle: "7: Linear Regression II"
read_date: "2024-02-20"
menu:
  content:
...
```{r lse-distributions, out.width="100%", fig.width=6, fig.height=3, echo=FALSE, warning = FALSE, message=FALSE}
library(gridExtra)
...
```{r father-son-regression, warning = FALSE, message = FALSE}
...
```{r do-tidy-example, warning=FALSE, message=FALSE}
dat %>%
  group_by(HR) %>%
...
```{r r-hat-hist, warning=FALSE, message=FALSE}
...
playerInfo <- Fielding %>%
  ...
  slice(1) %>%
  ungroup %>%
  left_join(People, by="playerID") %>% # updated to People in 2022
...
```{r regression-fallacy, echo=FALSE, fig.height=3, fig.width=5, out.width="40%"}
