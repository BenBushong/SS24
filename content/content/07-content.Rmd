...
Another handy tip is using `paste` with `collapse = '+'`.
...
We see a maximum correlation of `r round(max(res$r), 3)` and if you just plot the data from the group achieving this correlation, it shows a convincing plot that $X$ and $Y$ are in fact correlated:
...
```{r simpsons-regression, echo = TRUE}
...
```{r simpsons-regression-slope, echo = TRUE}
summary(lm(y ~ x*as.factor(z), data = dat))
```
