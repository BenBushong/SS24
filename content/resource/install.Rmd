...
Find the course assignment template (for SSC442, see the [Assignments](/assignment/) page), right-click to download it (don't just click on it - that won't work), open it in Rstudio, and without changing anything, click the "knit" button to test out your install. Your system should open a PDF that looks like something like this:

```{r,echo = FALSE}
knitr::include_graphics("/img/install/RenderedPDF.png", error = FALSE)
```
...
```{r, echo=F}
knitr::include_graphics("/img/install/Unicode.png", error = FALSE)
```
...
```{r, echo=F}
knitr::include_graphics("/img/install/UnicodeFix.png", error = FALSE)
```
...
```{r, echo=F}
knitr::include_graphics("/img/install/TexCapacityExceeded.png", error = FALSE)
```
...
```{r, echo=F}
knitr::include_graphics("/img/install/MacPermissions.png", error=FALSE)
```


**Why did it happen?**

The key here is that it says something about not having permission to write in `usr/bin`. This means your mac somehow thinks it can't write to the hard drive, or at least the folder that contains your R. It usually occurs when you're trying to update packages, or install `tinyTex`.

**How do I fix it**?

The error gives a link that tells you how to do it. It requires using `terminal` on your mac (hit command+space and type "terminal" to open the terminal). Then, follow the instructions at [the given link](https://github.com/rstudio/tinytex/issues/24). Type these two lines in verbatim into the terminal (don't copy-paste):

```{r, echo=F}
knitr::include_graphics("/img/install/FixMacPermissions1.png", error=FALSE)
```

If the error happened when you were trying to install `tinyTex`, then also do this step afterwards (also from the link):

```{r, echo=F}
knitr::include_graphics("/img/install/FixMacPermissions2.png", error=FALSE)
```
